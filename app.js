const data = [
    {
        type: ["入口網",],
        name: "Google",
        url: "https://google.com/",
        img: "image/web/42.jpg"
    },
    {
        type: ["入口網"],
        name: "Youtube",
        url: "https://www.youtube.com",
        img: "image/web/44.jpg",
    },
    {
        type: ["入口網",],
        name: "Facebook",
        url: "https://www.facebook.com/",
        img: "image/web/45.jpg",
    },
    {
        type: ["入口網",],
        name: "Yahoo",
        url: "https://tw.yahoo.com/",
        img: "image/web/43.jpg",
    },
    {
        type: ["入口網"],
        name: "PTT",
        url: "http://ppt.cc/",
        img: "image/web/003.jpg",
    },
    {
        type: ["入口網",],
        name: "Baidu 百度",
        url: "http://www.baidu.com/",
        img: "image/web/50.jpg",
    },
    {
        type: ["入口網",],
        name: "聯合新聞網",
        url: "https://udn.com/news/index/",
        img: "image/web/004.jpg",
    },
    {
        type: ["入口網",],
        name: "自由時報",
        url: "https://www.ltn.com.tw",
        img: "image/web/005.jpg",
    },
    {
        type: ["入口網",],
        name: "中時新聞網",
        url: "https://www.chinatimes.com/?chdtv",
        img: "image/web/006.jpg",
    },
    {
        type: ["入口網",],
        name: "新頭殼",
        url: "https://newtalk.tw/",
        img: "image/web/009.jpg",
    },
    {
        type: ["入口網",],
        name: "Nownews",
        url: "https://www.nownews.com/",
        img: "image/web/010.jpg",
    },
    {
        type: ["入口網",],
        name: "風傳媒",
        url: "https://www.storm.mg/",
        img: "image/web/011.jpg",
    },
    {
        type: ["入口網",],
        name: "三立新聞網",
        url: "https://www.setn.com/",
        img: "image/web/008.jpg",
    },
    {
        type: ["入口網",],
        name: "壹蘋新聞網",
        url: "https://tw.nextapple.com/",
        img: "image/web/007.jpg",
    },
    {
        type: ["入口網",],
        name: "TVBS NEWS",
        url: "https://www.tvbs.com.tw/",
        img: "image/web/012.jpg",
    },
    {
        type: ["入口網",],
        name: "CTWANT",
        url: "https://www.ctwant.com/",
        img: "image/web/013.jpg",
    },
    {
        type: ["入口網",],
        name: "ETtoday",
        url: "https://www.ettoday.net/",
        img: "image/web/014.jpg",
    },
    {
        type: ["入口網",],
        name: "鏡週刊",
        url: "https://www.mirrormedia.mg/",
        img: "image/web/015.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "小鴨影音",
        url: "https://777tv.app/",
        img: "image/web/1.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "Gimy TV",
        url: "https://gimys.tv/",
        img: "image/web/3.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "劇迷TV",
        url: "https://jumi.tv/",
        img: "image/web/4.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "BOWANG 播王",
        url: "https://bowang.su/",
        img: "image/web/54.jpg",
    },
    {
       type: ["綜合影劇Ａ",],
       name: "123kubo",
       url: "https://123kubo.tv/",
       img: "image/web/66.jpg",
   },
   {
       type: ["綜合影劇Ａ",],
       name: "GIKA 劇咖",
       url: "https://gika.tv",
       img: "image/web/70.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "Gimy 劇迷",
        url: "https://pttplay.cc/",
        img: "image/web/2.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "MoMoVOD",
        url: "https://momovod.app/",
        img: "image/web/7.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "Balatutu",
        url: "https://balatutu.me/",
        img: "image/web/8.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "片庫",
        url: "https://www.pkmp4.com/",
        img: "image/web/9.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "藍光影院",
        url: "https://www.lgyy.vip",
        img: "image/web/75.jpg",
    },   
    {
        type: ["綜合影劇Ａ",],
        name: "LoveTvShow",
        url: "https://lovetv.show",
        img: "image/web/13.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "Movieffm",
        url: "https://www.movieffm.net",
        img: "image/web/6.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "555電影",
        url: "https://www.555dianying.cc/",
        img: "image/web/10.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "五五影院",
        url: "https://www.55ccc.cc/",
        img: "image/web/55.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "94i影城",
        url: "https://94itv.app",
        img: "image/web/11.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "inmi 映迷",
        url: "https://www.inmi.app",
        img: "image/web/12.jpg",
    },
    {
        type: ["綜合影劇Ａ",],
        name: "黑料電影",
        url: "https://dy12.fun/catalog",
        img: "image/web/39.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "NO視頻",
        url: "https://www.novipnoad.com",
        img: "image/web/77.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "愛迪影視",
        url: "https://aidi.pro",
        img: "image/web/78.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "唐人街影院",
        url: "https://www.tangrenjie.tv",
        img: "image/web/84.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "小寶影院",
        url: "https://xiaobaotv.net/index.php",
        img: "image/web/82.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "329影城",
        url: "https://329tv.app",
        img: "image/web/83.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "99kubo酷播",
        url: "https://tv99kubo.tv/",
        img: "image/web/016.jpg",
    },                 
    {
        type: ["綜合影劇Ｂ",],
        name: "小鴨影音",
        url: "https://www.17vod.cc/",
        img: "image/web/15.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "IQTV VIP",
        url: "https://iqtv.vip",
        img: "image/web/69.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "我愛影視",
        url: "https://100.mvsky.xyz",
        img: "image/web/74.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "電影基地",
        url: "https://www.dyjidi.com",
        img: "image/web/72.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "努努影院",
        url: "https://www.nunuyy5.org/",
        img: "image/web/18.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "電影工廠",
        url: "https://dygc.cc",
        img: "image/web/76.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "海兔影院",
        url: "https://www.haitu.tv/",
        img: "image/web/81.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "欧乐影院",
        url: "https://www.olevod.com",
        img: "image/web/21.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "追心影視",
        url: "https://www.zhuixin.cc/",
        img: "image/web/79.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "53tv",
        url: "https://53tv.net/",
        img: "image/web/40.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "88影視網",
        url: "https://www.88mv.tv/",
        img: "image/web/56.jpg",
    },
    {
        type: ["綜合影劇Ｂ",],
        name: "大米星球",
        url: "https://dmxq.info",
        img: "image/web/71.jpg",
    },
    {
        type: ["綜合影劇Ｃ",],
        name: "楓林網",
        url: "https://8maple.tv/",
        img: "image/web/17.jpg",
    }, 
    {
        type: ["綜合影劇Ｃ",],
        name: "泥視頻",
        url: "https://www.nivod4.tv/index.html",
        img: "image/web/5.jpg",
    },
    {
        type: ["綜合影劇Ｃ",],
        name: "58BTV",
        url: "https://58btv.net/",
        img: "image/web/80.jpg",
    },    
    {
        type: ["綜合影劇Ｃ",],
        name: "愛壹帆",
        url: "https://www.iyf.tv",
        img: "image/web/68.jpg",
    },
    {
        type: ["綜合影劇Ｃ",],
        name: "愛壹帆清爽版",
        url: "https://www.iyf1.tv/",
        img: "image/web/67.jpg",
    }, 
    {
        type: ["綜合影劇Ｃ",],
        name: "MK影視",
        url: "https://www.mkvdo.com/",
        img: "image/web/58.jpg",
    },                   
    {
        type: ["綜合影劇Ｃ",],
        name: "小白影音",
        url: "https://www.maxbaytw.com/",
        img: "image/web/24.jpg",
    },
    {
        type: ["綜合影劇Ｃ",],
        name: "愛下載影院",
        url: "http://axz.la/",
        img: "image/web/73.jpg",
    },
    {
        type: ["綜合影劇Ｃ",],
        name: "99酷播",
        url: "https://123kubo.net/",
        img: "image/web/20.jpg",
    },     
    {
        type: ["綜合影劇Ｃ",],
        name: "伊莉影音",
        url: "https://eynytv.com",
        img: "image/web/22.jpg",
    },     
    {
        type: ["綜合影劇Ｃ",],
        name: "低端影視",
        url: "https://ddys.art/",
        img: "image/web/14.jpg",
    },    
    {
        type: ["綜合影劇Ｃ",],
        name: "狗狗影音",
        url: "https://dogevod.com/",
        img: "image/web/41.jpg",
    },
    {
        type: ["綜合影劇Ｃ",],
        name: "94580",
        url: "https://94580.net/",
        img: "image/web/16.jpg",
    },    
    {
        type: ["專題影劇",],
        name: "看韩剧",
        url: "https://www.kangii.com/",
        img: "image/web/26.jpg",
    },
    {
        type: ["專題影劇",],
        name: "韓劇網",
        url: "https://kortw.com/",
        img: "image/web/27.jpg",
    },
    {
        type: ["專題影劇",],
        name: "韓劇網",
        url: "https://6drama.com",
        img: "image/web/28.jpg",
    },
    {
        type: ["專題影劇",],
        name: "韓迷TV",
        url: "https://www.hanjutv.me/",
        img: "image/web/57.jpg",
    },
    {
        type: ["專題影劇",],
        name: "WeTV",
        url: "https://wetv.vip/zh-tw",
        img: "image/web/29.jpg",
    },
    {
        type: ["專題影劇",],
        name: "中國人線上看",
        url: "https://chinaq.io",
        img: "image/web/30.jpg",
    },
    {
        type: ["專題影劇",],
        name: "獨播庫",
        url: "https://www.duboku.tv/",
        img: "image/web/31.jpg",
    },
    {
        type: ["專題影劇",],
        name: "大劇獨播",
        url: "https://bigdramas.com/",
        img: "image/web/32.jpg",
    },
    {
        type: ["專題影劇",],
        name: "DramasQ",
        url: "https://dramasq.tv/",
        img: "image/web/33.jpg",
    },
    {
        type: ["專題影劇",],
        name: "櫻花動漫",
        url: "http://www.yinghuacd.com",
        img: "image/web/34.jpg",
    },
    {
        type: ["專題影劇",],
        name: "在線動畫",
        url: "https://www.agemys.net/",
        img: "image/web/35.jpg",
    },
    {
        type: ["專題影劇",],
        name: "D1DM 第一動漫",
        url: "https://d1-dm.online/",
        img: "image/web/36.jpg",
    },
    {
        type: ["專題影劇",],
        name: "嘀哩嘀哩",
        url: "https://dilidili.online/",
        img: "image/web/60.jpg",
    },
    {
        type: ["專題影劇",],
        name: "動畫線上看",
        url: "https://anime1.me/",
        img: "image/web/59.jpg",
    },
    {
        type: ["專題影劇",],
        name: "妖道角(布袋戲)",
        url: "https://video.yaodaojiao.com",
        img: "image/web/37.jpg",
    },
    {
        type: ["直播",],
        name: "ofiii 歐非",
        url: "https://www.ofiii.com/channel/watch/4gtv-4gtv051",
        img: "image/web/61.jpg",
    },
    {
        type: ["直播",],
        name: "TV線上看電視",
        url: "https://tv.wfuapp.com/2019/12/pts-live.html",
        img: "image/web/51.jpg",
    },
    {
        type: ["直播",],
        name: "四季線上 免費頻道",
        url: "https://www.4gtv.tv/channel/4gtv-4gtv003?set=4&ch=1",
        img: "image/web/64.jpg",
    },
    {
        type: ["直播",],
        name: "電視超人",
        url: "https://isuper.tv/taiwan-tv-live/",
        img: "image/web/62.jpg",
    },
    {
        type: ["直播",],
        name: "HiNet 免費影音",
        url: "https://www.hinet.net/tv/",
        img: "image/web/48.jpg",
    },
    {
        type: ["直播",],
        name: "Touch TTV 台視",
        url: "https://www.touchttv.com/touchttv/default.aspx",
        img: "image/web/49.jpg",
    },
    {
        type: ["直播",],
        name: "皮克直播",
        url: "https://piko.live/c/news",
        img: "image/web/52.jpg",
    },
    {
        type: ["直播",],
        name: "新聞台直播",
        url: "https://www.kocpc.com.tw/archives/12241",
        img: "image/web/63.jpg",
    },
    {
        type: ["直播",],
        name: "Twitch",
        url: "https://www.twitch.tv/",
        img: "image/web/004.png",
    },
    {
        type: ["直播",],
        name: "頻道密碼",
        url: "密碼.html",
        img: "image/web/65.jpg",
    },
    

];

const app = Vue.createApp({
    data(){
        return{
            filters: ['使用說明', '入口網', '綜合影劇Ａ', '綜合影劇Ｂ',  '綜合影劇Ｃ', '專題影劇', '直播'],
            selectedFilter: '使用說明',
            items: data,
            index: true,
        }
    },
    computed: {
        filteredItems() {
            if (this.selectedFilter === '使用說明') {
                this.index = true;
                return this.items === [];
            }
            return this.items.filter((item) => {
                this.index = false;
                if (Array.isArray(item.type)) {
                return item.type.includes(this.selectedFilter);
                } else {
                return item.type === this.selectedFilter;
                }
            });
        },
        },
        methods: {
        selectFilter(filter) {
            this.selectedFilter = filter;
        },
    },
});

app.mount("#app")